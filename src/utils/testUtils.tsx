...
/**
 * Check that a widget applies custom className
 * 
 * @param testId - Test ID of the widget root element
 * @param className - Expected class name
 */
export function expectCustomClassName(testId: string, className: string) {
  const element = screen.getByTestId(testId);
  expect(element).toHaveClass(className);
}

/**
 * Check that a widget uses custom testId
 * 
 * @param customTestId - Custom test ID that should be used
 */
export function expectCustomTestId(customTestId: string) {
  const element = screen.getByTestId(customTestId);
  expect(element).toBeInTheDocument();
}

/**
 * Standard test for BaseWidgetProps support
 * Tests that a widget properly handles className and testId props
 * 
 * @param Widget - React component to test
 * @param baseProps - Base props required by the widget
 * @param defaultTestId - Default test ID used by the widget
 * 
 * @example
 * testBaseWidgetProps(
 ...
 * );
 */
export function testBaseWidgetProps<P extends Record<string, any>>(
  Widget: React.ComponentType<P>,
  baseProps: P,
  defaultTestId: string
) {
  describe('BaseWidgetProps', () => {
    it('should apply custom className', () => {
      const customClass = 'custom-test-class';
      render(<Widget {...baseProps} className={customClass} />);
      
      const widget = screen.getByTestId(`widget-container-${defaultTestId}`);
      expect(widget).toHaveClass(customClass);
    });

    it('should use custom testId', () => {
      const customId = 'custom-widget-id';
      render(<Widget {...baseProps} testId={customId} />);
...
/**
 * Helper to create a spy console.error for testing error scenarios
 * Automatically restores console.error after the test
 * 
...
export const testDataFactories = {
  /**
   * Create test props with specific security level
   */
  createSecurityProps: (level: SecurityLevel) => ({
  ...
  /**
   * Create test props with mixed security levels
   */
  createMixedSecurityProps: (
