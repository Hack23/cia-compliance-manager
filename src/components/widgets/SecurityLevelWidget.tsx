...
import { SECURITY_LEVELS, WIDGET_ICONS, WIDGET_TITLES } from "../../constants/appConstants";
...
const SecurityLevelWidget: React.FC<SecurityLevelWidgetProps> = ({
  ...
  const [activeComponent, setActiveComponent] = useState<"availability" | "integrity" | "confidentiality">("availability");
  
  // Get details for the active component
  const activeDetails = ciaContentService.getComponentDetails(
    activeComponent,
    activeComponent === "availability" ? availabilityLevel :
    activeComponent === "integrity" ? integrityLevel :
    confidentialityLevel
  );
  ...
  const getComponentSummary = (component: string, level: SecurityLevel): string => {
    switch (component) {
      case "availability":
    ...
    >
      <div className="p-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            <h3 className="text-lg font-medium mb-4">Configure Security Levels</h3>
            ...
            <div className="space-y-4">
              ...
              >
                ...
                >
                  {availabilityLevel}: {getComponentSummary("availability", availabilityLevel)}
                </div>
              ...
              >
                ...
                >
                  {integrityLevel}: {getComponentSummary("integrity", integrityLevel)}
                </div>
              ...
              >
                ...
                >
                  {confidentialityLevel}: {getComponentSummary("confidentiality", confidentialityLevel)}
                </div>
          ...
          <div>
            <h3 className="text-lg font-medium mb-4">
              {activeComponent.charAt(0).toUpperCase() + activeComponent.slice(1)} Details
            </h3>
            
            <div className="p-4 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 h-full">
              {activeDetails ? (
                <div className="space-y-4">
                  <h4 className={`font-medium ${getComponentColor(activeComponent)}`}>
                    {activeComponent === "availability" ? availabilityLevel :
                     activeComponent === "integrity" ? integrityLevel :
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600 dark:text-gray-400">
                        {activeDetails.description || "No description available"}
                      </p>
                    </div>
                    
                    {/* Technical implementation */}
                    <div>
                      <h5 className="text-sm font-medium mb-1">Technical Implementation</h5>
                      <p className="text-sm text-gray-600 dark:text-gray-400">
                        {activeDetails.technical || "No technical details available"}
                      </p>
                    </div>
                    
                    {/* Business impact */}
                    <div>
                      <h5 className="text-sm font-medium mb-1">Business Impact</h5>
                      <p className="text-sm text-gray-600 dark:text-gray-400">
                        {activeDetails.businessImpact || "No business impact details available"}
                      </p>
                    </div>
                    ...
                    {activeComponent === "availability" && (
                      <div className="mt-4 grid grid-cols-2 gap-3">
                        <div className="bg-blue-50 dark:bg-blue-900 dark:bg-opacity-20 p-2 rounded">
                          <div className="text-xs font-medium mb-1 text-blue-700 dark:text-blue-300">Uptime</div>
                          <div className="text-sm">{activeDetails.uptime || "N/A"}</div>
                        </div>
                        <div className="bg-blue-50 dark:bg-blue-900 dark:bg-opacity-20 p-2 rounded">
                          <div className="text-xs font-medium mb-1 text-blue-700 dark:text-blue-300">Recovery Time</div>
                          <div className="text-sm">{activeDetails.rto || "N/A"}</div>
                        </div>
                      </div>
                    )}
                    
                    {activeComponent === "integrity" && activeDetails.validationMethod && (
                      <div className="mt-4 bg-green-50 dark:bg-green-900 dark:bg-opacity-20 p-2 rounded">
                        <div className="text-xs font-medium mb-1 text-green-700 dark:text-green-300">Validation Method</div>
                        <div className="text-sm">{activeDetails.validationMethod}</div>
                      </div>
                    )}
                    
                    {activeComponent === "confidentiality" && activeDetails.protectionMethod && (
                      <div className="mt-4 bg-purple-50 dark:bg-purple-900 dark:bg-opacity-20 p-2 rounded">
                        <div className="text-xs font-medium mb-1 text-purple-700 dark:text-purple-300">Protection Method</div>
                        <div className="text-sm">{activeDetails.protectionMethod}</div>
                      </div>
                    )}
                  </div>
                </div>
        ...
        <div className="mt-6 bg-gray-100 dark:bg-gray-800 p-4 rounded-lg">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-3">
            Higher security levels provide stronger protection but typically require more resources to implement and maintain.
            Consider your organization's needs and constraints when selecting security levels.
          </p>
          
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-2 text-xs">
            <div className="p-2 bg-red-50 dark:bg-red-900 dark:bg-opacity-20 rounded">
              <div className="font-medium text-red-700 dark:text-red-300">None</div>
              <div className="text-red-600 dark:text-red-400">Minimal to no security controls</div>
            </div>
            <div className="p-2 bg-yellow-50 dark:bg-yellow-900 dark:bg-opacity-20 rounded">
              <div className="font-medium text-yellow-700 dark:text-yellow-300">Low</div>
              <div className="text-yellow-600 dark:text-yellow-400">Basic security controls</div>
            </div>
            <div className="p-2 bg-blue-50 dark:bg-blue-900 dark:bg-opacity-20 rounded">
              <div className="font-medium text-blue-700 dark:text-blue-300">Moderate</div>
              <div className="text-blue-600 dark:text-blue-400">Standard security controls</div>
            </div>
            <div className="p-2 bg-green-50 dark:bg-green-900 dark:bg-opacity-20 rounded">
              ...
              <div className="text-green-600 dark:text-green-400">Advanced security controls</div>
            </div>
            <div className="p-2 bg-purple-50 dark:bg-purple-900 dark:bg-opacity-20 rounded">
              <div className="font-medium text-purple-700 dark:text-purple-300">Very High</div>
              <div className="text-purple-600 dark:text-purple-400">Maximum security controls</div>
            </div>
          </div>
        </div>
      </div>
