...
export function initPerformanceMonitoring() {
  ...
  recordPerformanceMetric('page-load', Date.now() - testStartTime, 'navigation');
...
export function recordPerformanceMetric(
  ...
  if (duration > 1000) {
    cy.log(`⚠️ SLOW OPERATION: ${operation} took ${duration.toFixed(2)}ms`);
  }
...
export function recordWidgetPerformance(
  ...
  if (renderTime > 500) {
    cy.log(`⚠️ SLOW WIDGET RENDER: ${widgetName} took ${renderTime.toFixed(2)}ms`);
  }
 ...
 */
export function generatePerformanceReport(): TestPerformanceReport {
  const now = Date.now();
  const totalDuration = now - testStartTime;
  
  // Calculate statistics
  const avgTime = currentTestMetrics.length > 0
    ? currentTestMetrics.reduce((sum, m) => sum + m.duration, 0) / currentTestMetrics.length
    : 0;
  ...
  const report: TestPerformanceReport = {
    ...
    testFile: Cypress.spec.relative || 'unknown',
    totalDuration,
...
export function createVisualPerformanceReport() {
  ...
  cy.window().then(win => {
    ...
    summary.innerHTML = `
      ...
      <p><strong>Average operation time:</strong> ${report.averageOperationTime.toFixed(2)}ms</p>
    ...
    const maxCategoryTime = Math.max(...categoryEntries.map(([_, time]) => time));
    ...
    report.slowestOperations.forEach(op => {
      ...
      row.innerHTML = `
        ...
        <td style="text-align: right; padding: 5px;">${op.duration.toFixed(2)}</td>
        <td style="text-align: left; padding: 5px;">${op.category || 'general'}</td>
      `;
...
Cypress.Commands.add('recordPerformanceMetric', (
  operation: string,
  duration: number,
  category?: string,
  metadata?: Record<string, any>
) => {
  recordPerformanceMetric(operation, duration, category, metadata);
  return cy.wrap(null);
});

Cypress.Commands.add('measureOperation', (
  fn: () => Cypress.Chainable<any>,
  operationName: string,
  category?: string
) => {
  return measureOperation(fn, operationName, category);
});
...
Cypress.Commands.add('endMeasurement', (label: string, category?: string): Cypress.Chainable<number> => {
  return cy.window().then(win => {
    const metrics = win.cypressPerformanceMetrics || { records: [], startTimes: {} };
    const startTime = metrics.startTimes?.[label];
    
    // Ensure we have a valid start time
    if (!startTime) {
      cy.log(`Warning: No start time found for measurement '${label}'. Using fallback.`);
